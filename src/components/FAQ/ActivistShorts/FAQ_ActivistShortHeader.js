...
const FAQ_ActivistShortHeader = (props) => {
  //let query = qs.parse(props.location.search, { ignoreQueryPrefix: true });
  const navItems = [
    { to: pathConst.ACTIVISTSHORTS_FAQ, search: '', name: 'FAQ', exact: true, disabled: false },
    { to: pathConst.ACTIVISTSHORTS_DEFINITION, search: '', name: 'Definitions', exact: false, disabled: false }
  ];
...
FAQ_ActivistShortHeader.propTypes = {
...
FAQ_ActivistShortHeader.defaultProps = {
  handleHoverSubmenu: () => {},
...
export default withRouter(React.memo(FAQ_ActivistShortHeader));
