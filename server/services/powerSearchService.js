...
const powersearchRoutes = 'powersearch';

class PowerSearchService {
  async getData(request, resultsCallback) {
    ...
    try {
      ...
      general.ErrorLogSql(`${powersearchRoutes}/PowerSearchService/DynamicSQL`, error, SQL);
    }
  }
  ...
  buildSql(request) {
    ...
    const filterJoinsSql = this.createAdvancedSearchFilteringJoins(request);
  ...
  createSelectSql(request) {
    ...
    let strTopByStatus = '';
    if (status === 2) {
      strTopByStatus = 'TOP 10';
    } else {
      strTopByStatus = 'TOP 50000';
    }
    ...
    if (this.isDoingGrouping(rowGroupCols, groupKeys)) {
      ...
      valueCols.forEach((valueCol) => {
        if ((valueCol.isShown && refreshButton) || !refreshButton) {
          switch (valueCol.aggFunc) {
            case 'count(Dist)':
              colsToSelect.push(`count (distinct ${valueCol.field}) as ''${valueCol.field}''`);
              break;
            case 'count':
              colsToSelect.push(`count (${valueCol.field}) as ''${valueCol.field}''`);
              break;
            case 'avg':
              colsToSelect.push(`avg (${valueCol.field}) as ''${valueCol.field}''`);
              break;
            case 'max':
              colsToSelect.push(`max (${valueCol.field}) as ''${valueCol.field}''`);
              break;
            case 'min':
              colsToSelect.push(`min (${valueCol.field}) as ''${valueCol.field}''`);
              break;
            case 'sum':
              colsToSelect.push(`sum (${valueCol.field}) as ''${valueCol.field}''`);
              break;
            default:
      ...
      return `select ${strTopByStatus} ${colsToSelect.join(', ')}`;
    }
    if (valueCols) {
      const arrFilterCheck = [];
      valueCols.forEach((valueCol) => {
        if ((valueCol.isShown && refreshButton) || !refreshButton) {
          arrFilterCheck.push(valueCol.field);
          colsToSelect.push(` ${valueCol.field} as ''${valueCol.field}''`);
        }
      });

      for (const [key, value] of Object.entries(filterModel)) {
        if (!arrFilterCheck.includes(key)) {
          colsToSelect.push(` ${key} as ''${key}''`);
        }
      }
  ...
  createFilterSql(key, item, index, operator) {
    ...
    if (item.operator) {
      ...
      switch (item.condition1.filterType) {
        case 'text':
          condition1 = this.createTextFilterSql(key, item.condition1, index, item.operator);
          break;
        case 'number':
          condition1 = this.createNumberFilterSql(key, item.condition1, index, item.operator);
          break;
        case 'date':
          condition1 = this.createDateFilterSql(key, item.condition1, index, item.operator);
          break;
        default:
      ...
      switch (item.condition2.filterType) {
        case 'text':
          condition2 = this.createTextFilterSql(key, item.condition2, index + 1, operator);
          break;
        case 'number':
          condition2 = this.createNumberFilterSql(key, item.condition2, index + 1, operator);
          break;
        case 'date':
          condition2 = this.createDateFilterSql(key, item.condition2, index + 1, operator);
          break;
        default:
          console.log(`unkonwn filter type: ${item.filterType}`);
      }
    ...
    switch (item.filterType) {
      case 'text':
        ...
        break;
      case 'number':
        conditionArr.push(this.createNumberFilterSql(key, item, index, operator));
        break;
      case 'date':
      ...
      case 'set':
        item.values.forEach((country, i) => {
          ...
          conditionArr.push(this.createTextFilterSql(key, thisItem, `${index}_${i}`, adjOperator));
        });
        break;
      default:
        console.log(`unkonwn filter type: ${item.filterType}`);
  ...
  createFromSql(request) {
    ...
    if (serverSideTables.joins) {
      serverSideTables.joins.forEach((join) => {
        ...
        tablesToJoin.push(`${join.type} join ${join.table} on ${join.on} = ${join.at}${additionalJoin}`);
      });
  ...
  createAdvancedSearchFilteringJoins(request) {
    const { dataContextFilter } = request;
    const { serverSideTables } = request;
    const arrFiltersToJoin = [];
    if (dataContextFilter) {
      for (const [key, value] of Object.entries(dataContextFilter.AdvancedSearchFilteringData)) {
        arrFiltersToJoin.push(` JOIN ${value.join}(${value.field_id}, ${value.list_id ? value.list_id : null}, ${value.min ? value.min : null}, ${value.max ? value.max : null}) fm${value.field_id} ON fm${value.field_id}.pid = ${serverSideTables.from}.pid`);
      }
    }
    return arrFiltersToJoin.join(' ');
  }
  ...
  createWhereSql(request, inline) {
    ...
    if (groupKeys.length > 0) {
      groupKeys.forEach((key, index) => {
        ...
        const createFilter = that.createFilterSql(key, colName, `gk_${rand}_${index}`, operator);
    ...
    if (filterModel) {
      ...
      keySet.forEach((key, index) => {
        ...
        if (item.isInline === inline) {
          ...
          const createFilter = that.createFilterSql(key, item, `${rand}_${index}`, operator);
          whereParts.push(...createFilter);
        }
      });
    }
  ...
  getRowCount(request, results) {
    if (results === null || results === undefined || results.recordset.length === 0) {
      return 0;
    }
  ...
  createParamatiseSqlSql(selectSql, fromSql, filterJoinsSql, whereSql, whereFinalSql, groupBySql, orderBySql, limitSql, request) {
    const { serverSideTables } = request;
    ...
    const whereSql2 = whereStatementArr.length > 0 ? ` where ${whereStatementArr.join(' ')}` : '';
    const finalWhere = finalWhereStatementArr.length > 0 ? ` ${whereStatementArr.length > 0 ? 'and' : 'where'} ${finalWhereStatementArr.join(' ')}` : '';
    const decalareSQL = decalareArr.length > 0 ? `, N'${decalareArr.join(', ')}'` : '';
    const columnSQL = columnArr.length > 0 ? `,${columnArr.join(', ')}` : '';

    return `EXECUTE sp_executesql N'${`${` drop table if exists #t select * into  #t ${fromSql}`} ${selectSql} from #t ${serverSideTables.from} ${filterJoinsSql}${whereSql2}${finalWhere}${groupBySql}${orderBySql}${limitSql}`}'${decalareSQL}${columnSQL}`;
  }
}

module.exports = { PowerSearchService };
