...
const ThisWeek = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.thisWeekNews) {
      setNewsIds(props.thisWeekNews.map((c) => c.news_id));
    }
  }, [query.newsID, getAll, isOpenedOn, props.thisWeekNews, newsDetailsArray.length]);
        ...
        <div className='col-xs-12 col-sm-12 col-lg-9 col-xl-9 p-0 position-reliatve'>
          {query.newsID !== undefined && isOpenedOn ? (
            ...
            <MultiNewsComponent
              ...
              staticPathConst={PathsConstant.NEWS_ACTIVISM_THIS_WEEK}
...
ThisWeek.propTypes = {
  thisWeekNews: PropTypes.array
};
ThisWeek.defaultProps = {
  thisWeekNews: []
};

export default React.memo(ThisWeek);
