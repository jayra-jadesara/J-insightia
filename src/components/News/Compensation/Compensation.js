...
const Compensation = (props) => {
  ...
  if (props.isDisableNewsCompensation) {
    return <Redirect to={PathsConstant.NEWS_OVERVIEW} />;
  }
  ...
  useEffect(() => {
    ...
    if (props.allLatestActivismNews) {
      setNewsIds(props.allLatestActivismNews.map((c) => c.news_id));
    }
  }, [query.newsID, getAll, isOpenedOn, props.allLatestActivismNews, newsDetailsArray.length]);
        ...
        <div className='col-xs-12 col-sm-12 col-lg-9 col-xl-9 p-0 position-reliatve'>
          {query.newsID !== undefined && isOpenedOn ? (
            ...
            <MultiNewsComponent
              ...
              staticPathConst={PathsConstant.NEWS_COMPENSATION}
...
Compensation.propTypes = {
  allLatestActivismNews: PropTypes.array
};
...
export default withRouter(React.memo(Compensation));
